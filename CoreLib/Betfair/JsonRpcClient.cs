...
namespace CoreLib.Betfair
{
    public class JsonRpcClient : HttpWebClientProtocol, IClient
    {
        ...
        private static System.Exception ReconstituteException(CoreLib.Betfair.TO.Exception ex)
        {
            var data = ex.Data;
            ...
            if (ex.Data != null)
            {
                ...
                exceptionData = data.Property(exceptionName).Value.ToString();
            }
            else
            {
                exceptionData = "Unknow error occurred";
            }
            return JsonConvert.Deserialize<APINGException>(exceptionData);
        }

        public IList<MarketProfitAndLoss> listMarketProfitAndLoss(IList<string> marketIds, bool includeSettledBets = true, bool includeBspBets = true, bool netOfCommission = false)
        {
            var args = new Dictionary<string, object>();
            args[MARKET_IDS] = marketIds;
